...
C23 introduces a true scoped enum syntax similar to C++:

```c
enum class Mode { READ, WRITE, APPEND };
```

Now `READ`, `WRITE`, and `APPEND` belong only to the `Mode` enum scope.
You'd reference them as `Mode::READ`, preventing name collisions and improving safety in modern C.
